...
vliquid <- dbGetQuery(con2,"
WITH FIS AS (SELECT FISCODIGO FROM TBFIS WHERE FISTPNATOP IN ('V','R','SR')
),

NF AS (SELECT EMPCODIGO,CLICODIGO,NFDTEMIS,NFCODIGO,NFLCFINANC FROM NOTAS
WHERE NFDTEMIS BETWEEN (CURRENT_DATE-1) - EXTRACT(DAY FROM (CURRENT_DATE-1)) + 1 AND 'TODAY'
AND NFSIT = 'N'
),
...
range_write("18AutvMNBd-jzrmvHs1eKVdlrii76W4gGHUzWRsX8wiU",data=vliquid,sheet = "DADOS",
            range = "A1") 
